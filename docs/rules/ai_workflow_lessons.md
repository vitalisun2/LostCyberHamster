...
- **Не объявлять задачу завершённой до конца git-цикла**: для `task/refactoring` минимум — commit + push ветки + merge в main + push main + чистый `git status` на рабочей ветке.
