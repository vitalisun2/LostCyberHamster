...
- [incubating] **Побочные изменения (автогенерация Unity, Addressables) должны попадать в main, а не только в integration-ветку.** Integration-ветка — стенд для проверки, не конечная точка. После завершения задачи main и integration/unity-live должны быть идентичны. Если автосгенерированные файлы изменились — коммитить сначала в main (или cherry-pick), затем синхронизировать integration.
