...
- После каждого шага рефакторинга `.cs`-файлов проверять, что затронутый `.csproj` содержит корректные `<Compile Include="..." />` записи для новых, удалённых и переименованных файлов.
...
- Удалять debug логи перед завершением правок, оставлять только `DiagLog` для важной диагностики.
...
Перед завершением правок проверить каждый затронутый файл (только изменённые, не весь проект):
...
2. **Язык summary:** XML-summary писать только на русском языке.
3. **Комментарии в теле метода:** если метод содержит 2+ логические единицы, каждая должна иметь краткий комментарий-заголовок (что делает блок, не как).
...
- По умолчанию не писать и не расширять тесты логики: unit, EditMode, PlayMode и editor-only тестовые harness'ы. Добавлять или менять такие тесты только по явному запросу пользователя или если задача прямо сформулирована как тестовая.
...
- При явном запросе запускать релевантные существующие EditMode/PlayMode тесты, если они уже есть.
