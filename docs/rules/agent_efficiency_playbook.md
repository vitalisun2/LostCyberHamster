...
- `regenerate_project_files` давать 30-60 секунд. При timeout проверить project root и request/response paths; не повторять слепо ту же команду.
- После реализации выполнять basic review и финальный C# gate только по правилам `docs/rules/code_conventions.md`.
- Ручную проверку поведения, Play Mode, UI, test level и computer-use validation не запускать и не имитировать.
- Для каждой feature/subfeature с ручной проверкой передавать пользователю короткий verification scenario со статусом `не выполнен пользователем` / `pending`; агент сам никогда не считает сценарий выполненным и может только отдельно зафиксировать явный результат пользователя.
- Разрешённые автоматизированные проверки запускать по одной команде; результат и точный evidence сообщать сразу, прежде чем переходить к следующей проверке.
- Правила basic review и рефакторинга брать из `docs/rules/code_conventions.md`.
...
### Передача ручной проверки пользователю

Каждый manual verification scenario оформлять кратко и отдельно для конкретной feature/subfeature:
...
- Сценарий описывает пользователю проверку, но не является evidence её выполнения: агент всегда передаёт его как pending и не объявляет выполненным; после обратной связи он только фиксирует результат, сообщённый пользователем.
