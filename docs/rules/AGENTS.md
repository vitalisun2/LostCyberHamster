...
6. `docs/refactoring/` и `docs/rules/clean_code_by_b_martin/README.md` — только для явно запрошенной задачи на архитектуру или большой самостоятельный рефакторинг. Для обычной реализации, basic review, bug fix, UI-задачи или commit-задачи эти материалы не нужны.
...
- Каждый агент в начале задачи активирует локальный skill `caveman` в режиме `full` (значение по умолчанию) и сохраняет его активным для всех ответов. Менять уровень или отключать режим можно только по явному запросу пользователя.
