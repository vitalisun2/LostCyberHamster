...
Каждая test-кнопка сама активирует Skateboard через charge `100` + `UltaEvent`. Scripted model затем отправляет `JumpRequest`/`SuperJumpRequest`. Ровно четыре combo-кнопки: `1+1+1`, `2+1`, `1+2`, `3 Combo`. `Super Jump` выбирает normal или super cycle. Runner при старте сам фиксирует `surface=Road` из stable `Run` или `surface=Roof` из stable `RoofRun` с живой support; другая surface блокирует кнопки. Snapshot входит в status/checklist и Enter Mode gate. Runner ждёт реальные `Ride / Jump / Landing`, проверяет impact depths, budget и natural completion.
