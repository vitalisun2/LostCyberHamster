...
- Если успешных leaf-веток нет, planner использует dead-end branch fallback: возвращает safe-prefix вместе с diagnosis, выбирая ветку теми же branch-priority правилами, что и successful selection.
...
- Для SwitchLane перед timed jump-on objective выбирать одно релевантное safe-window около deadline source-threat и ранний запуск внутри этого окна; `PlanEvaluator` не должен компенсировать timing/progress-кандидаты через first-trigger, final-index/projection fallback или score-bias, включая dead-end fallback.
