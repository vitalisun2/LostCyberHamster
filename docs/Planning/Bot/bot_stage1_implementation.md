...
* `SwitchLane` — если сам манёвр безопасен: во время смещения между линиями и в момент завершения смещения хомяк не сталкивается с Threat на целевой линии
* `Jump` — если тип перепрыгиваемый, есть ≥10 энергии, и сам прыжок с приземлением безопасен

**Определение safe на этапе 1:**

* unsafe — шаг приводит к потере жизни во время исполнения или в момент завершения шага
* угроза, которая после безопасного `SwitchLane` остаётся просто впереди на новой линии, не делает шаг unsafe и должна обрабатываться следующим пересчётом
...
* `SwitchLane`: непосредственно перед исполнением по живым данным проверяем
  immediate safety самого манёвра. Если live-данные показывают столкновение
  во время shift или в момент завершения shift — шаг отменяется
  (`WasCancelled = true`), оркестратор перезапускает pipeline.
