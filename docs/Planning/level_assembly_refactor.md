...
- `overrides` — точечные изменения конкретных препятствий в конкретном паттерне
- Два типа оверрайдов: `SpriteChange` (замена спрайта) и `Delete` (удаление препятствия)
...
Для каждого элемента patternSequence[i]:
  ...
  2. Для каждого obstacle в паттерне:
     a. Проверить overrides уровня:
        - Есть override типа Delete для этого obstacleId? → пропустить obstacle
        - Есть override типа SpriteChange? → использовать указанный spriteName
     
     b. Нет override? → Обратиться к Location Theme:
        - По obstacle.type найти маппинг для location уровня
        - Взять default спрайт
        - (Опционально: если sprites > 1, использовать seed для детерминированного выбора)
...
- Группировка по категориям (easy / medium / hard / special)
- Поиск по имени
...
### Сценарий 5: Удаление препятствия из паттерна на конкретном уровне
1. Открыть уровень, кликнуть на мешающий obstacle
2. Нажать «Delete override» → obstacle скрывается на этом уровне
3. В level JSON записывается `{ "obstacleId": X, "type": "Delete" }`
4. В других уровнях этот obstacle остаётся на месте
...
- [ ] **Категоризация паттернов:** Нужна ли формальная система категорий (easy/medium/hard) в PatternsCollection, или достаточно naming convention?
- [ ] **Seed-based randomness:** Реализовывать ли в первой итерации или отложить?
- [ ] **Оверрайд позиции:** Может ли override менять не только спрайт, но и x/y конкретного obstacle? Или это избыточно?
- [ ] **Multiple pattern instances:** Если один паттерн используется в уровне дважды (например, `easy_run` на позиции 0 и на позиции 5), как адресовать оверрайды? Ответ: по индексу в `patternSequence` (это индекс вхождения, а не индекс obstacle).
- [ ] **Совместимость Level Editor:** Нужно ли поддерживать оба формата (старый и новый) в переходный период?
