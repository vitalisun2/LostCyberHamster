...
namespace Assets.Scripts
{
    ...
    public static string BaseSettingsPath => Path.Combine(Application.persistentDataPath, "GameSettings");
        ...
        /// <summary>
        /// Доля ширины хомяка, которая должна перекрывать <b>маленькое живое</b> препятствие (smallAlive),
        /// чтобы прыжок считался удачным «раздавил + бонус».
        /// Перекрытие ≤ значения — хомяк лишь задевает препятствие и получает урон.
        /// </summary>
        public const float JumpOverlapCrushSmallThreshold = 0.5f;

        /// <summary>
        /// Доля ширины хомяка, которая должна перекрывать <b>большое живое</b> препятствие (bigAlive),
        /// чтобы прыжок считался удачным «раздавил + бонус».
        /// Перекрытие ≤ значения — хомяк лишь задевает препятствие и получает урон.
        /// </summary>
        public const float JumpOverlapCrushBigThreshold = 0.3f;
