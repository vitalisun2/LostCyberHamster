...
{
    /// <summary>
    /// Выполняет super roof-to-road jump-on как двухфазный runtime input.
    /// </summary>
    internal sealed class SuperJumpOnFromRoofExecutor : IActionExecutionHandler
    {
        /// <summary>
        ...
        public SuperJumpOnFromRoofExecutor(ActionTriggerGate triggerGate)
        {
        ...
        public ActionFireResult TryFire(Hamster hamster, PlannedAction action)
        {
            ...
            if (action.Kind != BotActionKind.SuperJumpOnFromRoof || !action.TargetObstacleInstanceId.HasValue)
            ...
            _upgradeReadyTime = Time.time + SuperJumpOnFromRoofTiming.UpgradeDelaySeconds;
