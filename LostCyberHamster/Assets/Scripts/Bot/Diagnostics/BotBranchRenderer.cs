...
{
    /// <summary>
    /// Рендерит в Game view всю выбранную ветку Bot.
    ...
    internal sealed class BotBranchRenderer
    {
        ...
        public void EnsureGlMaterial()
        {
            ...
            if (shader == null)
            {
                DebugManager.DiagLog("[Bot RENDER] Hidden/Internal-Colored shader not found");
                return;
            }
            ...
            out float rightX)
        {
            ...
            float elapsedShift = Mathf.Max(0f, Time.time - _previewStartTime) * BotConsts.GameSpeedBase;
