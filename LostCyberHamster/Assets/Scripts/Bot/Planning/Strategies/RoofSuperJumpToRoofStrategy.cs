...
{
    /// <summary>
    /// Стратегия RoofSuperJumpToRoof: суперпрыжком на другую крышу (bigNotAlive, mediumNotAlive).
    /// Skeleton реализация — пока возвращает "not implemented".
    /// </summary>
    public class RoofSuperJumpToRoofStrategy : IActionStrategy
    {
        private readonly float _superJumpLandingOffset;

        public RoofSuperJumpToRoofStrategy(float superJumpLandingOffset = BotConsts.SuperJumpLandingOffsetFallback)
        {
            _superJumpLandingOffset = superJumpLandingOffset;
        }

        public BotAction Action => BotAction.RoofSuperJumpToRoof;
