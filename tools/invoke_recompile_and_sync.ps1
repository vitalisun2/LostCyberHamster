<#
.SYNOPSIS
Performs the local code sync cycle after C# changes without launching a test level.

.DESCRIPTION
Requests Unity script recompilation, regenerates the Unity-generated .csproj/.sln files,
and reloads the already-open Visual Studio solution if that solution is currently open.
Use this when code changes must be reflected in Unity and in Visual Studio structure,
but no test level launch is needed.
#>
...
function Update-RunningVisualStudioSolution {
    param(
...
# while unfocused, the script wakes the editor window once as a fallback.
...
# with the actual file structure, then reload the matching already-open solution.
try {
    [void](Invoke-UnityAutomationCommand -Command 'regenerate_project_files' -RunningMessage 'project files regeneration')
    Update-RunningVisualStudioSolution -SolutionPath (Join-Path $projectPath 'LostCyberHamster.sln')
}
catch {
    Write-Host "[warn] regenerate_project_files failed: $($_.Exception.Message)"
}

Write-Host '[completed] Recompile and sync workflow finished.'
exit 0